...
{%block name="block_body"%}
    ...
    <div class="am-cf am-padding">
        <div class="am-fl am-cf">
            <strong><font size="4">设置：{%$cost_type_list[$rate_type]%}</font></strong>
            <br/><br/>
            <strong><font size="4">等级：{%$level_desc_list%}</font></strong>
        </div>
    </div>

    <div class="am-panel am-panel-default">
        <div class="am-panel-bd">
            <form class="am-form am-form-horizontal am-padding-0 am-cf" role="form" action="/hsmm/partnerCost">
    ...
    <section class="am-margin">
        ...
        <table id="editTable" class="am-table am-table-bordered am-table-centered">

            {%foreach $group_pattern_tab_map as $tabId => $tabItems%}

                <tr>
                    <th colspan="3" style="background-color: rgb(221,221,221)">价格反哺模式 【 {%$tabId%} 】</th>
                ...
                <tr class="table-title-tr" style="background-color: lightgoldenrodyellow">
                    <th rowspan="{%count($tabItems) + 1%}" class="am-text-middle" style="width: 8%;">成本结构</th>
                    <th style="width: 1%;">序号</th>
                    <th style="width: 8%;">{%$pattern_id_name_map[$tabId]%}</th>
                ...
                {%foreach $struct_list['partnerList'][$tabId] as $partnerId => $structList%}

                    {%if array_key_exists($partnerId, $tabItems)%}

                        <tr>
                            <td>{%$idx++%}</td>
                            <td>{%$tabItems[$partnerId]%}<br/>（{%$partnerId%}）</td>
                             {%*水平方向10个单元格*%}
                            {%foreach $_tmpFieldMap as $fieldId=>$fieldType%}
                                 {%*判断一下是否是可选择单位类型(带%和不带的) start*%}
                                {%if $fieldType=='2'%}
                                     {%*判断是否存在数据 start*%}
                                    {%if $structList['cost_list'][$fieldId]%}
                                        {%$_item=$structList['cost_list'][$fieldId]%}
                                        <td>
                                            <a class="edit-choice-type"
                                               href="javascript:;"
                                               data-cost_value_type="{%$_item['cost_value_type']%}"
                                               data-rate_type="{%$rate_type%}"
                                               data-pattern_tab_id="{%$tabId%}"
        ...
        $(function () {
            ...
            $('#editTable td a.edit-normal').editable({
                ...
                success: function (data, value) {
                    ...
                    var _data = {}
                    if (!checkVal(_val)) {
                        _data['rate_type'] = $(this).attr('data-rate_type')
