...
;(function(){
    ...
    $ ('.trigger-btn-apply').on ('click', function (e) {
        e.preventDefault ()
        if(_suning_iphone8_yuyue){
            window.location.href = '/wechathuodong/suningIphone8Yuyue?op_id='+_op_id+'&user_mobile='+_suning_iphone8_yuyue
            return
        }
        // 展示申请面板
        showApplyPanel ()
    ...
    function bindApplyFormEvent ($Form) {
        ...
        $Form.on ('submit', function (e) {
            ...
            $.ajax ({
                type     : 'POST',
                url      : $me.attr ('action') || '/wechathuodong/doYuyueSuningIphone8',
                data     : $me.serialize (),
                dataType : 'json',
                timeout  : 5000,
                success  : function (res) {
                    try {
                        ...
                        if (!res[ 'errno' ] || res['errno'] == '210') {

                            // 申请提交成功
                            window.location.href = '/wechathuodong/suningIphone8Yuyue?op_id='+_op_id+'&user_mobile='+$mobile.val()
    ...
    function showApplyPanel () {
        var
            st = $.tmpl ($ ('#JsSuningIphone8ApplyPanelTpl').html ()) (),
            theDialog = tcb.showDialog (st, 'suning-iphone8-apply-panel'),
    ...
    function showApplySuccessPanel () {
        var
            st = $.tmpl ($ ('#JsSuningMeilanNote6ApplySuccessPanelTpl').html ()) ()

        tcb.showDialog (st, 'suning-meilan-note6-apply-panel')
    }
