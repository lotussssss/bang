...
(function (root, factory) {
    ...
    function _format(data, options, parentKey) {
        ...
        if (type === BOOL) {
            ...
            if (data === "") {
                result = sn("span", STRING_EMPTY_CLASS_NAME, "(空)");
            } else {
            ...
            if (isSpecial) {
                ...
                result = sn("span", OBJ_EMPTY_CLASS_NAME, "(空)");
            } else {
            ...
            if (data.length > 0) {
                ...
                result = sn("span", ARRAY_EMPTY_CLASS_NAME, "(空)");
            }
        } else {
...
(function (root, factory) {
    ...
    if (typeof define === 'function' && define.amd) {
        ...
        root.JsonTable = factory();
    }
}(this, function () {
    "use strict";

    function format(item, json) {
        var html = _format(item, json);
        _render(item, html);
    }

    function formatAmazeUI(item, json) {
        var html = _format(item, json)
        if (html[0].tagName === 'TABLE') {
            html.addClass('am-table am-table-bordered am-text-xs am-margin-0 am-padding-0');
            html.find('table')
                    .addClass('am-table am-table-bordered am-text-xs am-margin-0 am-padding-0')
                    .css({'table-layout': 'fixed'});
        }
        html.find('span')
                .addClass('am-text-truncate')
                .css({'display': 'block'});
        _render(item, html);
    }

    function _format(item, json) {
        if (json) {
            try {
                var format = window.JsonHuman.format(json, {
