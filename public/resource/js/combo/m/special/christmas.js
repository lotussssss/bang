...
;/**import from `/resource/js/mobile/special/201612/christmas.js` **/
// 圣诞节活动
;
!function () {
    var
        ...
        wxData = {
            "title"   : '新年狂欢100%有奖，最高得iPad air2！',
            "desc"    : '同城帮优品2017新年狂欢，天天抽奖，天天好价！',
            "link"    : window.location.protocol + '//' + window.location.host + window.location.pathname,
            "imgUrl"  : 'https://p.ssl.qhimg.com/t01ae77c978897aa0e8.png',
    ...
    $ (function () {
        ...
        (function(){
            ...
            function drawBonusMap(canvas_dom){
                ...
                tcb.imageOnload('https://p3.ssl.qhmsg.com/t01e4e127c06d32e9a9.png', function(img){
            ...
            function rollBonusAtRemote(success, done){
                ...
                $.post('/youpin/doLotteryDoubleDan2016', params, function (res) {
        ...
        //====================================
        // 获取推荐商品列表
        function getPromoProductList () {

            $.get ('/youpin/aj_get_goods', {
                pn        : 0,
                page_size : 10
            }, function (res) {
                res = $.parseJSON (res);
                ...
                if (!res[ 'errno' ]) {
                    ...
                    var
                        $product_list = $ ('.block-xmas-promo-product-list'),
                        product_list = res[ 'result' ][ 'good_list' ]

                    // 限制显示商品数量
                    product_list.splice (10, 9999)

                    // 获取商品列表的html字符串
                    var html_fn = $.tmpl ($.trim ($ ("#JsM201612XmasPromoProductListTpl").html ())),
                        html_str = html_fn ({
                            'list'         : product_list,
                            'is_flash'     : false,
                            'is_today'     : false,
