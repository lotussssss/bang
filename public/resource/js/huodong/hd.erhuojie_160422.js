// 处理右边浮层广告在小窗口中的显示效果
function floatCardFixedWrap(){
    var wFloatWrap = W('.float-card-fixed');
    if ( !(wFloatWrap && wFloatWrap.length) ) {
        return ;
    }

    function setFloatCardFixedWrap(e){
        var client_x = 0;
    ...
    W(window).on('load', setFloatCardFixedWrap);
    W(window).on('resize', setFloatCardFixedWrap);
}
floatCardFixedWrap();
