!function () {
    var defaults = {
        $trigger: null,
        file_post_name: 'quality_picture',
        upload_url: '/m/doUpdateImg',
        upload_url_base64: '/quality_img/doUploadImg',
        capture: 'camera',
    ...
    function initTakePhotoUpload() {
        ...
        if (options.supportMultiple) {
            trigger_invoke_wrap += ' multiple="multiple"'
        }
    ...
    function uploadImg(formData, url) {
        ...
        callback_upload_before(me, formData.get(options.file_post_name), function () {
            ...
            $.ajax({
                ...
                contentType: false,
                headers: {
                    'Authorization' :  JSON.parse(localStorage.getItem('qualityImgUploadToken')) ? 'Bearer '+ JSON.parse(localStorage.getItem('qualityImgUploadToken')) : ''
                },
                data: formData,
    ...
    function getCompressedImg(img, file_type, callback) {
        ...
        EXIF.getData(img, function () {
            var imgObj = this
            var orientation = EXIF.getTag(imgObj, 'Orientation')
