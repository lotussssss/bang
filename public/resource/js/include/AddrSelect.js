(function(){
    ...
    function AddrSelect(options){
        ...
        var defaults = {
            'Id': ["AddrProvince", "AddrCity", "AddrArea"],
            'Opt0': ["省份", "城市", "区县"],
            'OptDefault': ['', '', '']
        };

        var me = this;
        me.options = QW.ObjectH.mix({}, defaults);
        me.options = QW.ObjectH.mix(me.options, options, true);

        var fn = AddrSelect.prototype;
        if (typeof fn.init === 'undefined') {
            ...
            fn.change = function(v){
                var me = this,
                    s = me.options['Id'],
                    opt0 = me.options['Opt0'];
                ...
                if (ss) {
                    with (ss) {
                        ...
                        if (++v < s.length) {me.change(v);}
                    }                    
                }
            };
            // 设置默认被选择的地址
            fn.preselect = function(OptDefault){
                var me = this,
                    OptDefault = OptDefault || me.options['OptDefault'],
                    Id = me.options['Id'];

                var index,
                    str = '0'; // 省、市、区县索引
                for (var i = 0; i < OptDefault.length; i++) {
                    // 当前遍历到的默认省、市、区县为空，直接停止后边的操作 退出方法
                    if(!OptDefault[i]){
                        return;
                    }
                    if (i>0) {
                        str += ('_' + (document.getElementById(Id[i-1]).selectedIndex - 1));
                    }

                    for (var ii = 0; ii < dsy.Items[str].length; ii++) {
                        if(dsy.Items[str][ii]==OptDefault[i]){
                            if (ii<dsy.Items[str].length) {
                                document.getElementById(Id[i]).selectedIndex = ii + 1;
                                me.change(i+1);
                            }
                            break;
                        }
    ...
    window.bang.AddrSelect = AddrSelect;
}());